...
public class BlurCapabilityHook implements IHook {
    ...
    public void onHook(final ClassLoader cl) throws Throwable {
        ...
        if (ctx != null && !ctx.getSharedPreferences(HookConfigs.PREFS_FILE, Context.MODE_PRIVATE)
            ...
            XLog.i(">> [特效补全]高级毛玻璃强制开启已关闭，跳过 Hook");
            return;
        }
        ...
        if (sdk < 33) {
            XLog.i(">> [特效补全]当前系统版本低于 Android 13，跳过高级毛玻璃注入 Hook");
            return;
        }
        ...
        try {
            ...
            if (sdk == 33) {
                ...
                XLog.i(">> [特效补全] 成功在 Android 13 注入高级材质参数");
            } else {
                // ============================================
                // 【Android 14 及以上专有逻辑 (sdk > 33)】：
                // 完美保留系统原生的高级材质软硬件配置，不对静态属性做任何侵入性篡改
                // ============================================
                XLog.i(">> [特效补全] 当前系统为 Android 14+");
            }

            // 4. 【Android 13 & 14+ 核心 Hook】
            //    直接 Hook 高级材质检测方法，阻断其读取 Settings.Secure 中的 background_blur_enable (0)，
            ...
            XLog.i(">> [特效补全] 成功跳过全局高级材质检测");

        } catch (Throwable t) {
            XLog.e(">> [特效补全] Hook出错", t);
        }
    }
}
